...
func (a *AddProductToCartSuite) SetupTest() {
	...
	a.productDAO.DeletAll()
	a.cartDAO.DeletAll()
	a.cartItemDAO.DeletAll()
	a.inventoryDAO.DeletAll()
}
...
func (a *AddProductToCartSuite) Test1() {
	a.Run("given that the product exists, when adding product to cart, then returns 204 and updates cart", func() {
		...
		a.productDAO.Create(daos.ProductSchema{
		...
		request := utils.GetOrThrow(http.NewRequest("POST", a.testEnvironment.BaseUrl()+"/v1/add-product-to-cart", strings.NewReader(`
			{
		...
		cartItemSchema := a.cartItemDAO.FindAllByCartId(uuid.MustParse("bb8357b2-b978-4675-9521-ef2da0bd1747"))
...
func (a *AddProductToCartSuite) Test2() {
	a.Run("given that the products exists, when adding product to cart multiple times, then returns 204 and updates cart", func() {
		...
		request1 := utils.GetOrThrow(http.NewRequest("POST", a.testEnvironment.BaseUrl()+"/v1/add-product-to-cart", strings.NewReader(`
			{
				"productId": "c0981e5b-9cb7-4623-9713-55db0317dc1a",
				"quantity": 8
			}
		...
		request2 := utils.GetOrThrow(http.NewRequest("POST", a.testEnvironment.BaseUrl()+"/v1/add-product-to-cart", strings.NewReader(`
			{
				"productId": "c0981e5b-9cb7-4623-9713-55db0317dc1a",
				"quantity": 4
			}
		...
		request3 := utils.GetOrThrow(http.NewRequest("POST", a.testEnvironment.BaseUrl()+"/v1/add-product-to-cart", strings.NewReader(`
			{
		...
		_ = utils.GetOrThrow(a.testEnvironment.Client().Do(request1))
		_ = utils.GetOrThrow(a.testEnvironment.Client().Do(request2))
		response := utils.GetOrThrow(a.testEnvironment.Client().Do(request3))
...
func (a *AddProductToCartSuite) Test6() {
	a.Run("when adding product to cart and body is invalid, then returns 400", func() {
		...
		for _, template := range templates {
			request := utils.GetOrThrow(http.NewRequest("POST", a.testEnvironment.BaseUrl()+"/v1/add-product-to-cart", strings.NewReader(template["body"])))
